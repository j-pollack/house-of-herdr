...
// Modifier mask bits (shared with keys.ts): 1=cmd 2=shift 4=alt
// 8=ctrl, applied as
// event flags so combos like cmd+shift+p work. Bare modifier keycodes
...
// terminal window the user is looking at when it has multiple windows.
...
static bool post_scroll(int32_t lines, double x_fraction, double y_fraction,
  ...
  ProcessSerialNumber front_process = {0, kNoProcess};
  pid_t front_pid = 0;
  const bool owner_is_front =
      found_window && GetFrontProcess(&front_process) == noErr &&
      GetProcessPID(&front_process, &front_pid) == noErr && front_pid == pid;
